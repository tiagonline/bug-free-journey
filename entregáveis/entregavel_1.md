...
A arquitetura descrita (Web App, API, Serviço Externo de OCR) apresenta 4 pontos críticos de falha:
...
4.  **Performance & Carga:** O cenário de pico (cargas médias e altas) pode sobrecarregar a API, o *storage* ou o serviço de OCR.
    * **Ponto importante (Paridade de Ambiente):** É crucial ajustar a máquina que será executada a homologação com a mesma capacidade da máquina de produção, mesmo que por um curto período de tempo, para garantir a validade dos testes de performance.
...
* **Revisão de Código (Shift-Left pelo QA):**
    * **Justificativa:** Para nivelamento do time e garantia de qualidade na origem (Shift-Left), é importante que *Pull Requests* (PRs) críticos sejam avaliados por pelo menos 50% do time de QAs, como já atuei na Minu.
...
* **Riscos Técnicos:**
    ...
    2.  **Inconsistência de Visualização:** O visualizador *inline* pode renderizar PDFs/JPEGs de forma diferente no Chrome vs. Safari.
        * **Mitigação (do meu CV):** Avaliar o uso de **Browserstack**  para testes cross-browser automatizados.
...
4.  **[Performance - Carga]** (Teste com K6). Simular 50 usuários fazendo uploads (5MB) simultaneamente (carga média/alta). 

* **Resultado Esperado:** O tempo de resposta da API de upload (P95) deve se manter aceitável e a taxa de erro deve ser 0%.
